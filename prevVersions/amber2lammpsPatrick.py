...
class Lammps:
    ...
    def Write_Lammps(self, Basename):
        ...
        L.append('LAMMPS data file for ' + self.name + '. CGCMM style.\n\n')
	...
	if self.dihedrals > 0:
        ...
        if self.impropers > 0:
            L.append(`self.improper_types` + ' improper types\n')
	L.append('\n')
        ...
        if self.atom_types != 0:
            ...
            for i in range(self.atom_types):
                L.append("{:3d} {:6.3f} # {:s}\n".format(i + 1, self.Masses[i], self.AtomTypes[i]))
        ...
        if self.impropers != 0 and self.improper_types != 0:
            L.append('Improper Coeffs\n\n')
            for i in range(self.improper_types):
                L.append(`i+1`)
                for j in range(len(self.Improper_Coeffs[0])):
                    L.append(' ' + `self.Improper_Coeffs[i][j]`)
                L.append('\n')
            L.append('\n')
        ...
        if self.atoms != 0:
            ...
            for i in range(self.atoms):
                ...
                L.append(' # ' + self.AtomTypes[self.Atoms[i][1] - 1])
                # L.append(' # ' + self.AtomNames[i])
                L.append('\n')
            L.append('\n')
        ...
        if self.impropers != 0 and self.improper_types != 0:
            L.append('Impropers\n\n')
            for i in range(self.impropers):
                L.append(`i+1`)
                for j in range(len(self.Impropers[0])):
                    L.append(' ' + `self.Impropers[i][j]`)
                L.append('\n')
            L.append('\n')
...
class Amber:
    ...
    def Coerce_to_Lammps(self):
        ...
        if self.CRD_is_read and self.TOP_is_read:
            ...
            for i in range(self.NPHIH):
                if self.SCEE[self.ICPH[i] - 1] == 0.0:
                    nimpropers += 1
            for i in range(self.MPHIA):
                if self.SCEE[self.ICP[i] - 1] == 0.0:
                    nimpropers += 1
            ...
            l.dihedrals = self.NPHIH + self.MPHIA - nimpropers
            l.impropers = nimpropers
            l.atom_types = self.NTYPES
            l.bond_types = self.NUMBND
            l.angle_types = self.NUMANG
            l.dihedral_types = self.NPTRA - self.SCEE.count(0.0)
            l.improper_types = self.SCEE.count(0.0)

            Shift = 0
            EngConn = 0.0433641 # conversion factor kcal/mol -> eV
            ...
            l.AtomTypes = {}
            for i in range(self.NATOM):
                l.AtomTypes[self.IAC[i] - 1] = self.ISYMBL[i]

            l.AtomNames = []
            for i in range(self.NATOM):
                l.AtomNames.append(self.IGRAPH[i])
            ...
            for i in range(self.NTYPES):
                ...
                if self.CN1[j] == 0.0:
                    ...
                    l.Nonbond_Coeffs[i][0] = \
                                EngConn * 0.25 * (self.CN2[j])**2 / self.CN1[j]
                if self.CN2[j] == 0.0:
            ...
            for i in range(self.NUMBND):
                l.Bond_Coeffs[i][0] = EngConn * self.RK[i]
            ...
            for i in range(self.NUMANG):
                l.Angle_Coeffs[i][0] = EngConn * self.TK[i]
                l.Angle_Coeffs[i][1] = (180/math.pi) * self.TEQ[i]

            l.Dihedral_Coeffs = []
            # Paddy for charmm style
            # for i in range(self.NPTRA):
            #     l.Dihedral_Coeffs.append([0,0,0])
            # for i in range(self.NPTRA):
            #     l.Dihedral_Coeffs[i][0] = EngConn * self.PK[i] / self.IDIVF * 2
            #     l.Dihedral_Coeffs[i][1] = int(self.PN[i])
            #     l.Dihedral_Coeffs[i][2] = self.PHASE[i]
            # for i in range(self.NPTRA):
            #     if self.SCEE[i] == 0.0:
            #         continue
            #     l.Dihedral_Coeffs.append([0,0,0])
            ...
            #     if self.SCEE[i] == 0.0:
            #         continue
            #     l.Dihedral_Coeffs[i][0] = EngConn * self.PK[i]
            #     if self.PHASE[i] == 0:
            ...
            for i in range(self.NPTRA):
                if self.SCEE[i] == 0.0:
                    continue
                l.Dihedral_Coeffs.append([EngConn * self.PK[i], 1 if self.PHASE[i] == 0 else -1, int(self.PN[i])])
            
            l.Improper_Coeffs = []
            for i in range(self.NPTRA):
                if self.SCEE[i] != 0.0:
                    continue
                l.Improper_Coeffs.append([EngConn * self.PK[i], 1 if self.PHASE[i] == 0 else -1, int(self.PN[i])])
            # for i in range(self.NPTRA):
            #     if self.SCEE[i] == 0.0:
            #         l.Improper_Coeffs.append([0,0,0])
            #         l.Improper_Coeffs[i][0] = EngConn * self.PK[i]
            #         if self.PHASE[i] == 0:
            #             l.Dihedral_Coeffs[i][1] = 1
            #         else:
            #             l.Dihedral_Coeffs[i][1] = -1
            ...
            for i in range(self.NPHIH):
                if self.SCEE[self.ICPH[i] - 1] == 0.0:
                    continue
                l.Dihedrals.append([self.ICPH[i], abs(self.IPH[i])/3 + 1, abs(self.JPH[i])/3 + 1, abs(self.KPH[i])/3 + 1, abs(self.LPH[i])/3 + 1])
            ...
            for i in range(self.NPHIA):
                if self.SCEE[self.ICP[i] - 1] == 0.0:
                    continue
                l.Dihedrals.append([self.ICP[i], abs(self.IP[i])/3 + 1, abs(self.JP[i])/3 + 1, abs(self.KP[i])/3 + 1, abs(self.LP[i])/3 + 1])
            ...
            for i in range(self.NPHIH):
                if self.SCEE[self.ICPH[i] - 1] != 0.0:
                    continue
                l.Impropers.append([self.ICPH[i] - l.dihedral_types, abs(self.IPH[i])/3 + 1, abs(self.JPH[i])/3 + 1, abs(self.KPH[i])/3 + 1, abs(self.LPH[i])/3 + 1])
            for i in range(self.NPHIA):
                if self.SCEE[self.ICP[i] - 1] != 0.0:
                    continue
                l.Impropers.append([self.ICP[i] - l.dihedral_types, abs(self.IP[i])/3 + 1, abs(self.JP[i])/3 + 1, abs(self.KP[i])/3 + 1, abs(self.LP[i])/3 + 1])
	...
	print self.ITITL # Printing Self Title
        ...
        self.NUMEXTRA = eval(Pop(Item_list,0))
        ...
        if len(Item_list) < 5 * self.NATOM + self.NTYPES**2 + \
           ...
           3*self.NPTRA + self.NATYP:
            print '(errorA: File too short!)'
            return -1

        self.IGRAPH = []
#        Pop(Item_list,0) # WHY??? /Paddy
	...
	print 'Reading Charges...'
        ...
        print 'Reading Atomic Numbers...'
	self.ATNUM = []
        for i in range(self.NATOM):
            self.ATNUM.append(eval(Pop(Item_list,0)))
	...
	print 'Reading Dihedral Phase...'
        ...
        print 'Reading SCEE Scaling Factors...'
        self.SCEE = []
        for i in range(self.NPTRA):
            self.SCEE.append(eval(Pop(Item_list,0)))

        print 'Reading SCNB Scaling Factors...'
        self.SCNB = []
        for i in range(self.NPTRA):
            self.SCNB.append(eval(Pop(Item_list,0)))
...
#            self.ISYMBL.append(Pop(Item_list,0))
	...
	print 'Reading IRotate...' # Currently unused in Sander and Gibbs
        ...
        print 'Reading Radius Description String...'
        self.RADSET = Pop(Item_list,0) + " " + Pop(Item_list,0) + " " + Pop(Item_list,0) + " " + Pop(Item_list,0)
        # this fails in the case of mbondi3. /Paddy

        print 'Reading Radii...'
        self.RADII = []
        for i in range(self.NATOM):
            self.RADII.append(eval(Pop(Item_list,0)))

        print 'Reading Screen...'
        self.SCREEN = []
        for i in range(self.NATOM):
            self.SCREEN.append(eval(Pop(Item_list,0)))

        if self.IFPERT > 0:
            if len(Item_list) < 4 * self.NBPER + 5 * self.NGPER + \
               6 * self.NDPER + self.NRES + 6 * self.NATOM:
                print '(errorH: File too short!)'
                return -1
